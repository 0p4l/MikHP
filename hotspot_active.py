...
def hActive():
    ...
    try:
        ...
        print("Retrieving active hotspot users...")
        hactive = list(api('/ip/hotspot/active/print'))

        # Display active hotspot users
        result = []

        result.append("===== Hotspot Aktif =====")
        if not hactive:
            result.append("Tidak ada data pengguna hotspot aktif.")
        else:
            for user in hactive:
                result.append(f"ID: {user.get('.id', 'N/A')}")
                result.append(f"Server: {user.get('server', 'N/A')}")
                result.append(f"User: {user.get('user', 'N/A')}")
                result.append(f"Domain: {user.get('domain', 'N/A')}")
                result.append(f"Address: {user.get('address', 'N/A')}")
                result.append(f"MAC Address: {user.get('mac-address', 'N/A')}")
                result.append(f"Login By: {user.get('login-by', 'N/A')}")
